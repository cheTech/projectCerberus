...
class texttospeech_Api(object):
    ...
    def __say(self, text, engine):  # сказать
        print("texttospeech_Api: Said %s with %s" % (transliterate.translit(text, 'ru', reversed=True), engine))  # лог
        if engine == "yandex":  # через Yandex SpeechKit Api
            playsound(self.__getVoiceFromYandex(text=text, fileName=self.pathtotmp))
    ...
    def say(self, text, engine="yandex"):  # добавить фразу в очередь на синтез
        Process(target=self.__say, args=(text, engine))
